...
public async subscribe(followerId: ObjectId, followingId: ObjectId): Promise<Follower> {
    if (followerId.toString() === followingId.toString()) { // object ID larni premoy solishtrgan bn referenci har xil buladi shunga stringga aylantirib olamiz
      throw new InternalServerErrorException(Message.SELF_SUBSCRIPTION_DENIED);
    }
  
    const targetMember = await this.memberService.getMember(null, followingId); // null berilmasa 1 ta view qushib quyadi
